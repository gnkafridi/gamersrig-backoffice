...
<title>Order {{ $order->order_number }} – GamersRig</title>
...
<div class="card">
  ...
  <div class="meta">
    <div class="bill-to">
      ...
      @php
        ...
        $address = trim(($order->customer->address ?? '') . ($order->customer->city ? ', ' . $order->customer->city : ''), ', ');
      @endphp
      <div class="cust-row">
    ...
    <div class="meta-right">
      ...
      @if($order->payment_method)
        <div class="meta-line"><strong>Payment Method:</strong> {{ $order->payment_method }}</div>
      @endif
      <hr class="meta-rule">
      @php
  ...
  <div class="totals-section">
    <div class="totals-box">
      <div class="trow">
        ...
        <span class="amount">Rs. {{ number_format($order->subtotal, 0) }}</span>
      </div>
      @if(floatval($order->discount) > 0)
      <div class="trow">
        <span class="label">Discount</span>
        <span class="amount discount">− Rs. {{ number_format($order->discount, 0) }}</span>
      </div>
      @endif
      @if(floatval($order->tax) > 0)
      <div class="trow">
        <span class="label">Tax</span>
        <span class="amount">Rs. {{ number_format($order->tax, 0) }}</span>
      </div>
      @endif
      @if(floatval($order->delivery_fee) > 0)
      <div class="trow">
        <span class="label">Delivery Fee</span>
        <span class="amount">Rs. {{ number_format($order->delivery_fee, 0) }}</span>
      </div>
      @endif
  ...
  {{-- Notes --}}
  @php $noteText = $order->notes ?: 'No Return and Warranty on any item until the mentioned.'; @endphp
